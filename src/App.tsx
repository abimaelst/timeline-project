  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-error">
        ...
        <p>{error}</p>
        <button onClick={fetchTimelineItems} className="btn btn-error btn-sm mt-4">
          Try Again
        </button>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={fetchTimelineItems} className="btn btn-ghost btn-sm">
          Refresh
        </button>
      </div>
      <Timeline initialItems={items} onItemUpdate={handleTimelineUpdate} />
    </div>
  );
};
